...
to V3** — throughput tuning, parallel consumers, and `max.poll.records`
sizing are a separate axis from the durability and ordering work in V2.
...
| OPS-53 | `notification-service` **MUST NOT** maintain its own idempotency table. Upstream idempotency in `fulfillment-service` (§3.9) is the source of truth; notification-side dedup is V3 hardening. *(Known V2 limitation: a relay-crash-mid-publish window can cause one duplicate notification per crash. See §5 and §7 Q14.)* |
...
| OPS-115 | The outbox relay **MUST** run in the same JVM process as the fulfillment consumer. *(Splitting the relay into a separate sidecar is V3 architectural work.)* |
...
| OPS-123 | V2 routes a record to DLQ on the **first** parse failure. Retry-before-DLQ (a separate retry topic with bounded attempts) is V3 hardening. |
...
- **F5 — consumer lag under burst load.** V2 is correctness-focused; throughput
  tuning (parallel consumers, `max.poll.records`, partition count revisit) is
  V3 territory. The F5 chaos test continues to assert the V1 lag baseline in V2.
- **Notification-side idempotency.** A relay-crash-mid-publish window can
  produce one duplicate notification. Closing it requires either Kafka native
  transactions across DB+Kafka (rejected in §7 Q9) or a `processed_notifications`
  table in `notification-service` (V3 hardening).
- Retry-before-DLQ (a separate retry topic with bounded attempts).
- Saga pattern with compensating actions (payment → inventory → ship).
- Distributed tracing (Build 5).
- Prometheus metrics (V3).
- Distributed outbox relay (multi-instance, leader election).
...
| F5 | Consumer lag under burst load | 50 ms work × single thread → linear lag growth | **Not addressed in V2.** Throughput work deferred to V3. | — |
...
fix; F5 assertion stays as-is and is the V3 entry point.
...
| F5 | `F5_ConsumerLagTest` | `assertTrue(lag > 200)` | unchanged in V2 — V3 will invert |
...
| Q9 | Kafka native transactions or DB-backed outbox? | **DB-backed outbox.** Decouples DB durability from Kafka availability; avoids transactional-producer complexity and the DB↔Kafka coordination problem. Consequence: notification-side dedup is V3, not V2. |
| Q10 | Outbox relay deployment? | **In-process inside `fulfillment-service`** as a separate thread sharing the connection pool. Splitting the relay into a sidecar process is V3 architectural work. |
| Q11 | Retry-before-DLQ? | **Straight to DLQ on first parse failure.** Retry topics with bounded attempts are V3 hardening. |
...
| Q14 | F5 (consumer lag) in V2 scope? | **Deferred to V3.** V2 is correctness; F5 is throughput. F5 chaos test stays asserting the V1 lag baseline; V3 inverts it. |
...
| 0.2 | 2026-04-28 | Steve Weiland | V2: Postgres + idempotency table (F1, F2 fix), transactional outbox + manual commit (F2, F3 fix), DLQ for poison messages (F4 fix). F5 deferred to V3. |
